...
<body>
	...
	//the object of the snake
		var snake = {
			flag:0,//weather crash the food  0--no 1--yes
			speed:300,
			direction:'right',
			body:[],
			count:0,
			eat:function(i){
				ground.food[i].dom.className = 'block snake-block';
				ground.food[i].dom.style.left = this.head.offsetLeft + 'px';
				ground.food[i].dom.style.top = this.head.offsetTop + 'px'; 
				this.head.style.left = this.nextpos.ileft + 'px';
				this.head.style.top = this.nextpos.itop + 'px';
			 	this.body.splice(1,0,ground.food[i].dom);
			 	if(this.count == 3)
			 	{
			 		for(var i =0;i<3;i++){
			 			ground.food[i] = ground.createFood();
			 		}
			 		this.count =0;
			 	}
			 	//this.flag =0;		 	
			...
			for(var i =0;i<3;i++){
			if(this.nextpos.ileft == ground.food[i].pos.left && this.nextpos.itop == ground.food[i].pos.top){
				this.count ++;
				this.flag = 1;
				this.n = i;
			}
	...
	//create the food
			for(var i =0;i<3;i++){
			ground.food[i] = ground.createFood();//make a attribute ,food ,for the ground
		}
